...
public class CircularMenu extends JPanel {
	...
	public CircularMenu(Color couleur, int width, int height, int rayon, int taille, List<JComponent> items) {
	...
	private void build() {
		if (items != null && items.size() > 0) {
			...
			if (items.size() < 9) {
				...
				int shift = 1;
				for (int i = 8; i < items.size(); i++, shift += 2) {
					items.get(i).setBounds((int) (xCentre - taille / 2 + rayon * Math.cos(4 * angle - Math.PI / 2)),
							(int) (yCentre - taille / 2 + rayon * Math.sin(4 * angle - Math.PI / 2)) + taille
									+ (taille * shift),
							taille + 10, taille + 10);
					add(items.get(i));
				}
